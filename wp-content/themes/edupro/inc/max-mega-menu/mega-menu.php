...
add_filter( 'megamenu_nav_menu_args', 'edupro_megamenu_nav_menu_args', 10, 3 );
function edupro_megamenu_nav_menu_args( $defaults, $menu_id, $current_theme_location ) {
	...
	$defaults['menu_class']   = 'menu edupro-max-megamenu';
	$defaults['menu_id']      = '';
	$defaults['walker']       = new Edupro_Mega_Menu_Walker();

	return $defaults;
}

class Edupro_Mega_Menu_Walker extends Mega_Menu_Walker {
	...
	function start_lvl( &$output, $depth = 0, $args = array() ) {
		...
		$output .= "\n$indent<ul class=\"mega-sub-menu $class\">\n";
	}
	...
	function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ( $item->type == 'widget' && $item->content ) {
			...
			$item_output .=( 'on' == $edupro_megamenu_enabled && 'megamenu' != $max_megamenu_type ) ? '' : '<a'. $attributes .'>';
			...
			$item_output .=( 'on' == $edupro_megamenu_enabled && 'megamenu' != $max_megamenu_type ) ? '' : '</a>';
...
// custom max megamenu
function edupro_megamenu_add_theme_default($themes) {
...
.header_style3 #mega-menu-menu-1 > li > a:after {
...
.header--s4 .header__search,
.header--s4 .social-icon,
.header--s5 .header__search,
.header--s5 .social-icon,
.header--s6 .header__search,
.header--s6 .social-icon,
.header--s7 .header__search,
.header--s7 .social-icon,
...
add_filter("megamenu_themes", "edupro_megamenu_add_theme_default");
