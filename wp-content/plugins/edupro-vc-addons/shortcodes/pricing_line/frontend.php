...
// Button class
$unique_class = 'pricing-line--' . uniqid();
...
$icon = '';
if ( !empty( $atts['icon'] ) ) {
	$class .= " pricing-line--icon pricing-line--icon-{$atts['icon_position']}";
	$icon = "<i class='{$atts['icon']}'></i>";
}

$button = sprintf(
	'<a href="%s" class="pricing-line %s" target="%s" rel="%s">%s%s</a>',
	esc_url( $link['url'] ),
	strip_tags( $class ),
...
if ( !empty( $atts['no_link'] ) ) {
	$button = sprintf(
		'<div class="pricing_no_link %s">%s%s</div>',
		strip_tags( $class ),
		$icon && 'right' == $atts['icon_position'] ? esc_html( $link['title'] ) : $icon,
		$icon && 'right' == $atts['icon_position'] ? $icon : esc_html( $link['title'] )
	);
